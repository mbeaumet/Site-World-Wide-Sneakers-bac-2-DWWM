...
switch($method["opt"]){
    ...
    case 'select_id':
        ...
        case 'update_id':
            if (isset($_POST["sneakers_id"],$_POST["users_id"],$_POST["size"],$_POST["color"],$_POST["brand"],$_POST["states"],$_POST["price"], $_POST["image"],$_POST["stock"]) && !empty(trim($_POST["size"])) && !empty(trim($_POST["color"])) && !empty(trim($_POST["brand"])) && !empty(trim($_POST["states"])) && !empty(trim($_POST["price"])) && !empty(trim($_POST["image"])) && !empty(trim($_POST["stock"]))){
                $req= $db->prepare("UPDATE sneakers SET size=?,color=?,brand=?,states=?,price=?,image=?,stock=? WHERE users_id=?");
                $req->execute([$_POST["size"],$_POST["color"],$_POST["brand"],$_POST["states"],$_POST["price"],$_POST["image"],$_POST["stock"], $_SESSION("users_id")]);
                echo json_encode(["success"=>true]);
            }else{
