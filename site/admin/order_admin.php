...
switch($method['opt']){
    ...
    case 'insert':
        //print_r($_POST);
        //print_r($_SESSION);
        if(isset($_POST["etat_c"],$_POST["date_de_commande"],$_SESSION["users_id"],$_POST["id_sneakers"]) && !empty(trim($_POST["etat_c"]) && !empty(trim($_POST["date_de_commande"])) && !empty(trim($_POST["users_id"])) && !empty(trim($_POST["id_sneakers"])))){
    ...
    case 'update':
        if(isset($_SESSION["users_id"],$_POST["id_sneakers"],$_POST["etat_c"],$_POST["date_de_commande"]) && !empty(trim($_SESSION["users_id"])) && !empty(trim($_POST["id_sneakers"])) && !empty(trim($_POST["etat_c"])) && !empty(trim($_POST["date_de_commande"]))){
            $req=$db->prepare("UPDATE (etat_c = ?) FROM orders");
            $req->execute([$_POST["etat_c"]]);
    ...
    case 'delete':
        if(isset($_POST["etat_c"]) && !empty(trim($_POST["etat_c"]))){
            $req=$db->prepare("DELETE FROM orders WHERE etat_c = ?");
            $req->execute([$_POST["etat_c"]]);
